...
2. **YouTube & Shorts Scraper**: Pengambilan otomatis komentar, nama penulis, jumlah Likes (votes), dan waktu unggah relatif serta timestamp dari tautan video reguler maupun Shorts YouTube tanpa memerlukan API Key YouTube yang rumit. Mendukung penarikan data dinamis hingga maksimal 1000 komentar (default 100 komentar).
3. **Interactive Ground Truth Editor & Auto-Fill**: Dashboard menyediakan antarmuka tabel interaktif (menggunakan st.data_editor) untuk melakukan anotasi Ground Truth secara langsung di browser. Untuk mempermudah pelabelan, sistem secara otomatis mengisi nilai Ground Truth yang kosong dengan hasil prediksi LLM (yang memiliki tingkat akurasi kontekstual lebih tinggi), baik saat penarikan data awal maupun melalui tombol pengisian otomatis massal.
...
6. **Filter Perbedaan Prediksi (Prediction Mismatch Filter) & Highlight Warna**: Dashboard dilengkapi penyaring data untuk menyembunyikan komentar yang memiliki kesepakatan model agar pengguna fokus melakukan anotasi pada data mismatch. Sel Ground Truth yang memiliki perbedaan prediksi antara Lexicon dan LLM akan otomatis diwarnai kuning/oranye (warning highlight) di dashboard, berkas ekspor Excel, maupun laporan PDF untuk mempermudah pemeriksaan manual.
...
12. **Ekspor Laporan PDF & Excel**: Pengguna dapat mengunduh lembar kerja Excel hasil analisis yang terformat rapi dan mencetak laporan analisis komprehensif berformat PDF yang mencakup visualisasi grafik, Confusion Matrix, dan ringkasan metrik evaluasi. Semua berkas ekspor mendukung pewarnaan mismatch secara otomatis.
